...
const OrdersPage = () => {
  ...
  const fetchOrders = React.useCallback(async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://online-shopping-7fom.onrender.com'}/api/orders/myorders`, {
  ...
  const seedMockOrders = async () => {
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || 'https://online-shopping-7fom.onrender.com'}/api/orders/seed`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchOrders();
    } catch (err) {
